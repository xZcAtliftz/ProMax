...
-- [[ Example This Ui Here !! ]]

local Royx = Library.new()

local Tab1 = Royx:CreateTab("Tab 1")

local Tab2 = Royx:CreateTab("Tab 2 eiei")

local Page1 = Tab1:CreatePage("Main",1)

local Page2 = Tab1:CreatePage("Main",2)

local ReLa = Page1:AddLabel({Title = "Text Here !"})

Page1:AddToggle({Title = "Toggle",Default = false,callback = function (v)
	print(v)
end})

Page1:AddButton({Title = "Refresh Label",callback = function (v)
	ReLa:Options().Text = "Refresh Now !"
end})

Page1:AddButton({Title = "Refresh Color",callback = function (v)
	ReLa.UpdateColor(Color3.fromRGB(255,0,0))
end})

Page1:AddToggleButton({Title = "Toggle Button",Default = true,callback = function (v)
	print(v)
end})

Page1:AddMultiDropdown({Title = "MultiDropdown",Item = {"1","2","3"},Default = {"1"},callback = function (v)
	print(v)
end})

local redrop = Page1:AddDropdown({Title = "Dropdown",Item = {"1","2","3"},callback = function (v)
	print(v)
end})

Page1:AddButton({Title = "Refresh Dropdown",callback = function (v)
	redrop:Clear()
end})

Page1:AddButton({Title = "Add Dropdown",callback = function (v)
	redrop:Add("A")
	redrop:Add("B")
	redrop:Add("C")
end})

local sli = Page1:AddSlider({Title = "Slider",Min = 1,Default = 50,Max = 100,function (v)
	print(v)
end})

Page1:AddButton({Title = "Update Slider",callback = function (v)
	sli.Update(20)
end})

Page1:AddTextBox({Title = "TextBox",Placeholder = "Placeholder",callback = function (v)
	print(v)
end})
